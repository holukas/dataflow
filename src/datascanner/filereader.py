...
class FileReader():
    ...
    def __init__(self, fileinfo, filetypeconf, logger, nrows=None):
        self.filepath = fileinfo['filepath']
        self.config_filetype = fileinfo['config_filetype']
    ...
    def _special_format_icosseq(self):
        ...
        subformat = None
        if '-PRF-' in self.config_filetype:
            subformat = 'PRF'
        if '-CMB-' in self.config_filetype:
            subformat = 'CMB'
    ...
    # def _detect_skiprows(self):
    #     """Detect indices of rows with bad data that can then be used in .read_csv
    #     Does not seem to work consistently, not used at the moment"""
    #     checkcol_s = self.data_df.iloc[:, 0].copy()
    #     filter_goodrows = checkcol_s == self.good_rows_id
    #     filter_badrows = ~filter_goodrows
    #     skiprows = checkcol_s[filter_badrows].index.to_list()
    #
    #     num_goodrows = filter_goodrows.sum()
    #     num_badrows = filter_badrows.sum()
    #
    #     # badrows_nan_s = checkcol_s.where(checkcol_s == self.good_rows_id)
    #     # filter_badrows = badrows_nan_s.isnull()
    #     # rowlist_badrows = checkcol_s[filter_badrows].index.to_list()
    #     # skiprows = rowlist_badrows  # Indices list of bad rows
    #
    #     # skiprows = badrows_nan_s[badrows_nan_s.isnull()].index.to_list()  # Indices list of bad rows
    #
    #     # filter_goodrows = badrows_nan_s == self.good_rows_id
    #     # num_goodrows = filter_goodrows.sum()
    #     # filter_badrows = badrows_nan_s != self.good_rows_id
    #     # num_badrows = filter_badrows.sum()
    #     return skiprows, num_goodrows
